...
console.log(dubl([1,2,3]));
console.log(dubl([5,1,2,3,10]));
...
*  Test Case 2: [2,10]
*/
function onlyEve(arr){
  arr.forEach(myFunction)
  console.log(arr);

  function myFunction(item, index, ar) {
      ar[index] = item % 2 === 0 ? item : null;
  }
}
console.log(onlyEve([1,2,3]));
console.log(onlyEve([5,1,2,3,10]));
...
function showFirstAndLast(arr){
  ...
  function myFunction(item, index, ar) {
      ar[index] = item[0]+item[item.length - 1];
  }
}
...
console.log(dubl([{name: 'Elie'}, {name: 'Tim'}, {name: 'Matt'}, {name: 'Colt'}], 'title', 'instructor'));
